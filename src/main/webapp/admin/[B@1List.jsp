...
<div id="content" style="width: 930px;">
  ...
  <section>
    <strong><i style="background:#428BD1;">&nbsp;</i>&nbsp;文章列表</strong>
...
<table style="width: 100%;">
	...
	<tr id='post_${foo.id}'>
		...
		<td class='title'><a href="${basePath}/post/${foo.id}.html" target="_blank">${foo.title}</a></td>
		<td class='catalog'>${foo.catalog.name}</td>
		<td><fmt:formatDate value="${foo.createTime}" pattern="yyyy-MM-dd HH:mm" /></td>
		<td class='opts'>
			<a href='${basePath}/admin/list/?tp=delete_&postId=${foo.id}' onclick="return confirm('确认要删除吗');">删除</a>
			<a href='${basePath}/admin/edit-blog/?id=${foo.id}' title='点击编辑文章'>编辑</a>
		</td>
	</tr>
	</c:forEach>
